...
namespace SharpNeatLib.NeuralNetwork
{
	...
	public class FloatFastConcurrentNetwork : INetwork
	{
        ...
        public void SingleStep()
        {
            ...
            unsafe
            {
                fixed (FloatFastConnection* fixedConnection = connectionArray)
                {
                    fixed (float* fixedNeuronSignal = neuronSignalArray)
                    {
                        fixed (float* _fixedNeuronSignal = _neuronSignalArray)
                        {
                            FloatFastConnection* connection = fixedConnection;
                            float* neuronSignal = fixedNeuronSignal;
                            float* _neuronSignal = _fixedNeuronSignal;

                            int i = connectionArray.Length;

                            while (--i >= 0)
                            {
                                *(_fixedNeuronSignal + connection->targetNeuronIdx) +=
                                    (connection->signal = *(fixedNeuronSignal + connection->sourceNeuronIdx) * connection->weight);
                                connection++;
                            }

                            int length = _neuronSignalArray.Length;
                            i = totalInputNeuronCount;

                            neuronSignal = fixedNeuronSignal + i;
                            _neuronSignal = _fixedNeuronSignal + i;

                            do
                            {
                                *neuronSignal = activationFnArray[i].Calculate(*_neuronSignal);
                                neuronSignal++;
                                *(_neuronSignal++) = 0.0F;
                            }
                            while (++i < length);
